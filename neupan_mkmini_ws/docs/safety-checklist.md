...
- [ ] 指定一名不操作开发电脑的独立安全员。
- [ ] 确认测试区域已与无关人员隔离。
- [ ] 确认 NeuPAN 安全桥最低非零输出为 `0.5 m/s`、硬上限为 `0.6 m/s`；底盘 SDK `/cmd_vel` 适配器使用相同上限，NeuPAN 控制期间不得绕过安全桥。
- [ ] 确认倒车功能已禁用。
- [ ] 确认 `/ctrl_cmd` 只有一个发布者。
...
- [ ] 确认 FAST_LIO `/Odometry` 正常，底盘 `/odom` 仅作诊断且 `publish_odom_tf=false`。
- [ ] 现场 `fast_lio_tf.yaml` 已填入实测 `base_link -> livox_frame` 外参并设为 `calibrated: true`；仓库不包含校准值。
...
3. 确认未解锁时输出速度为零。
4. 解锁安全桥，并发送持续时间小于一秒的 `0.5 m/s` 命令。
5. 确认前进档反馈为 `4`。
...
- 架空轮和首次落地直线测试使用 `0.5 m/s`；验证通过后仅增加到 `0.6 m/s`，不得超过硬上限。
- 始终保留至少 2 m 的无障碍停车空间。
...
- [ ] `config/robots/mkmini/robot.yaml` 的 `planner_config_file` 与 `dune_checkpoint_file` 均可解析且文件存在。
